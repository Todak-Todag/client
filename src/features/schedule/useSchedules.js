...
/** 서비스 종류 ID → 이름. 실패해도 목록은 보여줄 수 있도록 빈 Map을 돌려준다. */
async function loadServiceNames(signal) {
  try {
    // 서비스 종류는 수가 적어 한 페이지(최대 50)로 충분하다
    const page = await getProvideServices({ size: 50, signal })
    return new Map(
      (page?.content ?? []).map((item) => [item.provideServiceId, item.provideServiceName]),
    )
  } catch (error) {
    if (signal.aborted) throw error
    return new Map()
...
async function loadTodaySchedules(signal) {
  const page = await getSchedules({ date: toLocalDateString(), size: 50, signal })
  ...
  const [names, provideServiceIds] = await Promise.all([
    loadServiceNames(signal),
    Promise.all(
  ...
  return schedules.map((schedule, index) => ({
    ...schedule,
    serviceName: names.get(provideServiceIds[index]) ?? null,
  }))
}
 ...
 *   startedAt: string, finishedAt: string, serviceName: string|null }> | null,
